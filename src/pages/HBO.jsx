...
const HBO = () => {
    ...
    useEffect(() => {
        const fetchInitialContent = async () => {
            try {
                // Max provider ID is 1899, HBO is 118
                const moviePromises = [];
                const tvPromises = [];

                for (let page = 1; page <= 5; page++) {
                    moviePromises.push(
                        fetch(`/api/discover/movie?with_watch_providers=1899|118&watch_region=US&sort_by=popularity.desc&page=${page}`)
                    );
                    tvPromises.push(
                        fetch(`/api/discover/tv?with_watch_providers=1899|118&watch_region=US&sort_by=popularity.desc&page=${page}`)
                    );
                }
    ...
    const loadMoreMovies = async () => {
        ...
        try {
            ...
            for (let page = startPage; page <= endPage; page++) {
                nextPages.push(fetch(`/api/discover/movie?with_watch_providers=1899|118&watch_region=US&sort_by=popularity.desc&page=${page}`));
            }
    ...
    const loadMoreTV = async () => {
        ...
        try {
            ...
            for (let page = startPage; page <= endPage; page++) {
                nextPages.push(fetch(`/api/discover/tv?with_watch_providers=1899|118&watch_region=US&sort_by=popularity.desc&page=${page}`));
            }
    ...
    const handleItemClick = (item) => {
        navigate(`/watch?type=${item.media_type || 'movie'}&id=${item.id}`);
    };
            ...
            <div className="hbo-page">
                <div className="hbo-page-header">
                    ...
                    <div className="hbo-page-title-section">
                        <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/1/17/HBO_Max_Logo.svg/1280px-HBO_Max_Logo.svg.png" alt="HBO" className="hbo-page-logo" />
                        <h1 className="hbo-page-title">HBO Originals</h1>
                    </div>
                </div>
                <div className="hbo-page-loading">
                    <div className="loading-spinner"></div>
                    <p>Loading HBO content...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="hbo-page">
            <div className="hbo-page-header">
                ...
                <p className="hbo-page-subtitle">Popular movies and TV shows available on HBO Max</p>
            </div>
            ...
            <section className="hbo-content-section">
                <div className="hbo-section-header">
                    ...
                    <span className="hbo-section-count">{movies.length} titles</span>
                </div>
            ...
            <section className="hbo-content-section">
                <div className="hbo-section-header">
                    ...
                    <span className="hbo-section-count">{tvShows.length} titles</span>
                </div>
                <div className="hbo-grid">
                    {tvShows.map(show => (
