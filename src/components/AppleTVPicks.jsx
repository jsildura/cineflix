...
const AppleTVPicks = () => {
    ...
    // Fetch Apple TV+ content based on selected genre
    const fetchAppleTVContent = async (genre = null) => {
        try {
            setLoading(true);

            // Apple TV+ provider ID is 350 in TMDB
            let movieUrl = '/api/discover/movie?with_watch_providers=350&watch_region=US&sort_by=popularity.desc';
            let tvUrl = '/api/discover/tv?with_watch_providers=350&watch_region=US&sort_by=popularity.desc';
            ...
            if (!moviesRes.ok || !tvRes.ok) {
                throw new Error('Failed to fetch Apple TV+ content');
            }
    ...
    useEffect(() => {
        fetchAppleTVContent(selectedGenre);
    }, [selectedGenre]);
    ...
    return (
        <div className="appletv-section">
            {/* Header with Apple TV+ branding */}
            <div className="appletv-header">
                <div className="appletv-header-left">
                    <div className="appletv-header-accent"></div>
                    <h2 className="appletv-title">Apple TV Originals</h2>
                    <img
                        src="https://upload.wikimedia.org/wikipedia/en/e/eb/Apple_TV_logo_2025.png"
                        alt="Apple TV+"
                        className="appletv-logo"
                    />
                </div>

                {/* Genre Filter Buttons */}
                <div className="appletv-genre-filters">
                    <button
                        className={`appletv-genre-btn ${selectedGenre === 'action' ? 'active' : ''}`}
                    ...
                    <button
                        className={`appletv-genre-btn ${selectedGenre === 'romance' ? 'active' : ''}`}
                    ...
                    <button
                        className={`appletv-genre-btn ${selectedGenre === 'comedy' ? 'active' : ''}`}
                    ...
                    <button
                        className={`appletv-genre-btn ${selectedGenre === 'horror' ? 'active' : ''}`}
                ...
                <Link to="/apple-tv" className="appletv-view-all">
                    View all
            ...
            {/* Carousel */}
            {loading ? (
                <div className="appletv-loading">
                    <div className="loading-spinner"></div>
                </div>
            ) : (
                <div className="appletv-carousel" role="region" aria-roledescription="carousel">
                    <div className="appletv-carousel-viewport">
                        <div
                            className="appletv-carousel-track"
                            style={{ transform: `translate3d(-${translateX}%, 0px, 0px)` }}
                        >
                            {content.map((item) => (
                                <div
                                    key={`${item.media_type}-${item.id}`}
                                    className="appletv-carousel-slide"
                                    role="group"
                                    aria-roledescription="slide"
                                >
                                    <div
                                        className="appletv-card"
                                    ...
                                    >
                                        <div className="appletv-card-gradient"></div>
                                        <img
                                            src={item.backdrop_path ? `${BACKDROP_URL}${item.backdrop_path}` : '/placeholder-backdrop.jpg'}
                                            alt={item.title || item.name}
                                            className="appletv-card-image"
                                            loading="lazy"
                                        />
                                        <div className="appletv-card-info">
                                            <h3 className="appletv-card-title">{item.title || item.name}</h3>
                                        </div>
                                    </div>
                                </div>
                            ))}
                    ...
                    {/* Navigation buttons */}
                    <button
                        className="appletv-carousel-btn appletv-carousel-prev"
                    ...
                    <button
                        className="appletv-carousel-btn appletv-carousel-next"
                        onClick={handleNext}
