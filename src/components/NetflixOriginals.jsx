...
const NetflixOriginals = () => {
    ...
    // Fetch Netflix content based on selected genre
    const fetchNetflixContent = async (genre = null) => {
        try {
            setLoading(true);

            let movieUrl = '/api/discover/movie?with_watch_providers=8&watch_region=US&sort_by=popularity.desc';
            let tvUrl = '/api/discover/tv?with_watch_providers=8&watch_region=US&sort_by=popularity.desc';
            ...
            if (!moviesRes.ok || !tvRes.ok) {
                throw new Error('Failed to fetch Netflix content');
            }
    ...
    useEffect(() => {
        fetchNetflixContent(selectedGenre);
    }, [selectedGenre]);
    ...
    return (
        <div className="netflix-originals">
            ...
            <div className="netflix-originals-header">
                <div className="netflix-header-left">
                    <div className="netflix-header-accent"></div>
                    <h2 className="netflix-originals-title">Netflix Originals</h2>
                </div>

                {/* Genre Filter Buttons */}
                <div className="netflix-genre-filters">
                    <button
                        className={`netflix-genre-btn ${selectedGenre === 'action' ? 'active' : ''}`}
                    ...
                    <button
                        className={`netflix-genre-btn ${selectedGenre === 'romance' ? 'active' : ''}`}
                    ...
                    <button
                        className={`netflix-genre-btn ${selectedGenre === 'comedy' ? 'active' : ''}`}
                    ...
                    <button
                        className={`netflix-genre-btn ${selectedGenre === 'horror' ? 'active' : ''}`}
                ...
                <Link to="/netflix" className="netflix-view-all">
                    View all
            ...
            {loading ? (
                ...
                <div className="netflix-carousel" role="region" aria-roledescription="carousel">
                    <div className="netflix-carousel-viewport">
                        <div
                            className="netflix-carousel-track"
                            style={{ transform: `translate3d(-${translateX}%, 0px, 0px)` }}
                        >
                            {content.map((item) => (
                                <div
                                    key={`${item.media_type}-${item.id}`}
                                    className="netflix-carousel-slide"
                                    role="group"
                                ...
                                >
                                    ...
                                    >
                                        <div className="netflix-movie-gradient"></div>
                                        <img
                                        ...
                                        <div className="netflix-movie-info">
                                            <h3 className="netflix-movie-title">{item.title || item.name}</h3>
                                        </div>
                                    </div>
                                </div>
                            ))}
                    ...
                    {/* Navigation buttons */}
                    <button
                        className="netflix-carousel-btn netflix-carousel-prev"
                    ...
                    <button
                        className="netflix-carousel-btn netflix-carousel-next"
                        onClick={handleNext}
